...
// File contracts/AeroDexLiquidity.sol
...
contract AeroDexLiquidity is Ownable {
    ...
    function addLiquidity(
        ...
        // Approve Aerodrome Router (calculating inline)
        IERC20(tokenA).safeIncreaseAllowance(AERO_ROUTER, amountADesired - ((amountADesired * feeBps) / 10000));
        IERC20(tokenB).safeIncreaseAllowance(AERO_ROUTER, amountBDesired - ((amountBDesired * feeBps) / 10000));

        // Execute Add Liquidity on Aerodrome Router
        (amountA, amountB, liquidity) = IAeroRouter(AERO_ROUTER).addLiquidity(
            tokenA,
            tokenB,
            stable,
    ...
    function addLiquidityETH(
        ...
        // Approve Aerodrome Router (calculating inline)
        IERC20(token).safeIncreaseAllowance(AERO_ROUTER, amountTokenDesired - ((amountTokenDesired * feeBps) / 10000));

        // Execute Add Liquidity ETH on Aerodrome Router
        (amountToken, amountETH, liquidity) = IAeroRouter(AERO_ROUTER).addLiquidityETH{
